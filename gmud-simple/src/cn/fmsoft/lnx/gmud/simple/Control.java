...
public class Control extends View {
	...
	/** 允许在按下后自动重复的按键掩码 */
	private final static int REPEAT_KEY = Input.kKeyLeft | Input.kKeyRight;

	/** 重复按钮消息 */
	private RepeatRunnable mRepeatKey = new RepeatRunnable();

	/** 重复按键机制：周期调用 {@link Input#GmudSetKey(int)} 发送指定的键值，并且周期逐步变短 */
	private class RepeatRunnable implements Runnable {
		/** 基础延时 */
		private final int DELAY_BASE = 180;
		/** 延时渐变 */
		private final int DELAY_RATE = 7;
		/** 延时阀值 */
		private final int DELAY_FIANL = 50;
		private int mDelay = DELAY_BASE;
		private int mKey = 0;

		/** 设置需要重复的按键。如果成功，则启动重复循环 */
		void setKey(int key) {
			key &= REPEAT_KEY;
			if (key != 0 && mKey == 0) {
				mDelay = DELAY_BASE;
		...
		@Override
		public void run() {
			if ((mKey & REPEAT_KEY) != 0) {
				Input.GmudSetKey(mKey);
				if (mDelay > DELAY_FIANL)
					mDelay -= DELAY_RATE;
	...
	public boolean dispatchKeyEventPreIme(KeyEvent event) {
		...
		if (Input.Running) {
			...
			if (KeyEvent.KEYCODE_ENTER == code
					|| (KeyEvent.KEYCODE_0 <= code && code <= KeyEvent.KEYCODE_Z)) {
	...
	void drawSoftKey(Canvas canvas) {
		...
		for (int i = 0; i < 9; i++) {
			...
			canvas.drawRect(sSoftKey[i], paint);
			canvas.drawText(sSoftKeyTitle[i], sSoftKey[i].centerX(),
					sSoftKey[i].centerY(), paint);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (action == MotionEvent.ACTION_DOWN
			...
			mRepeatKey.setKey(action == MotionEvent.ACTION_DOWN ? mask : 0);
